                      ...
                      <ul className="tags">
                        {
                          question.tags.split(',').map((tag, index) => (
                            <li key={String(index)}><Link to="#">{tag}</Link></li>
                          ))
                        }                   
                      </ul>
                      <span className="posted-on">Posted on <Link to="#">{formatDate(question.created_at)}
                        </Link> by <Link to={`/profile/${question.username}`}>{question.username}</Link>
                      </span>
                    </div>
                  </div>
                ))
...
const mapStateToProps = state => ({ questions: state.home.results, hotQuestions: state.sidebar.results });
